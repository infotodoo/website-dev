...
class ForceWebsite(WebsiteForm):
    @http.route('/website/force_website/<int:website_id>', type='http', website=True, auth="user")
    def force_website(self, website_id, **kwargs):
        request.env['website']._force_website(website_id)
        #website_obj = request.env['website'].sudo().browse(website_id)
        #request.website = website_obj
